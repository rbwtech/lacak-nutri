...
export const useOwnerAuth = (showToast, fetchItems) => {
  ...
  const _performApiCall = async (type, id, data, config = {}) => {
  ...
  const handleWriteOperation = async (actionType, id = null, data = null) => {
    if (isOwnerAdmin()) {
      setPendingAction({
        type: actionType,
        id: id,
      ...
      await _performApiCall(actionType, id, data);
      return true;
    }
  };
  ...
  const executePendingAction = useCallback(
    async (code) => {
      ...
      await _performApiCall(type, id, data, config);
